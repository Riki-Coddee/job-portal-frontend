...
import {
  ...
  Send, Loader2, AlertCircle, Award,
  Car, Languages, Globe, GraduationCap,
  Plus, Trash2, CheckCircle, XCircle, AlertTriangle, Star
} from 'lucide-react';
...
${name}`;
  ...
  const calculateMatchScore = useCallback(() => {
    ...
    if (formData.custom_skills.length > 0 && jobRequirements) {
      ...
      totalScore += matchingCustomSkills.length * 3;
      ...
      if (i === 0) {
        stars.push(
          <button
            ...
            className={`p-2 sm:p-1 ${currentRating === 0 ? 'text-gray-400' : 'text-gray-300'} hover:text-gray-500`}
            title="Clear rating"
        ...
        stars.push(
          <button
            ...
            className={`p-2 sm:p-1 ${i <= currentRating ? 'text-yellow-400' : 'text-gray-300'} hover:text-yellow-500`}
            title={`Rate ${i} stars`}
      ...
      <AnimatePresence>
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex min-h-screen items-center justify-center p-2 sm:p-4">
            ...
            >
              {/* Header */}
              <div className="sticky top-0 z-10 bg-white border-b border-gray-200 rounded-t-2xl p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                  <div>
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900">Apply for {jobTitle}</h2>
                    <p className="text-sm sm:text-base text-gray-600 mt-1">Complete your application</p>
                  </div>
                  <div className="flex items-center justify-between sm:justify-end space-x-2 sm:space-x-4">
                    {/* Match Score Display */}
                    <div className="bg-gradient-to-r from-blue-50 to-indigo-50 px-3 sm:px-4 py-2 rounded-lg border border-blue-200 flex-shrink-0">
                      <div className="flex items-center space-x-2">
                        <Award className="h-4 w-4 sm:h-5 sm:w-5 text-blue-600" />
                        <div>
                          <div className="text-xs sm:text-sm text-gray-600">Match</div>
                          <div className="text-base sm:text-lg font-bold text-blue-700">{matchScore}%</div>
                        </div>
                      </div>
                    </div>
                    <button
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6">
                {loading ? (
                  <div className="p-8 sm:p-12 text-center">
                    <Loader2 className="animate-spin h-10 w-10 sm:h-12 sm:w-12 text-primary-600 mx-auto" />
                  ...
                  <div className="space-y-6 sm:space-y-8">
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 sm:p-4">
                      <h3 className="font-semibold text-blue-900 mb-2 sm:mb-3 flex items-center text-sm sm:text-base">
                        <User size={16} sm:size={18} className="mr-2 flex-shrink-0" />
                        Your Application Information
                      </h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3 text-xs sm:text-sm">
                        <div className="flex items-center">
                          <Mail size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                          <span className="text-blue-800 truncate">{profile?.user?.email}</span>
                        </div>
                        {profile?.phone_number && (
                          <div className="flex items-center">
                            <Phone size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                            <span className="text-blue-800 truncate">{profile.phone_number}</span>
                          </div>
                        )}
                        <div className="flex items-center">
                          <Briefcase size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                          <span className="text-blue-800 truncate">
                            {profile?.title || 'No professional title'}
                          </span>
                        </div>
                        <div className="flex items-center">
                          <FileText size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                          <span className="text-blue-800 truncate">
                            {profile?.resume ? 'Resume available' : 'No resume uploaded'}
                          </span>
                        </div>
                        <div className="flex items-center">
                          <GraduationCap size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                          <span className="text-blue-800 truncate">
                            {profile?.educations?.length || 0} education records
                          </span>
                        </div>
                        <div className="flex items-center">
                          <Award size={14} sm:size={16} className="mr-2 text-blue-600 flex-shrink-0" />
                          <span className="text-blue-800 truncate">
                      ...
                      {!profile?.resume && (
                        <div className="mt-3 p-2 sm:p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                          <div className="flex items-start">
                            <AlertCircle size={14} sm:size={16} className="text-yellow-600 mr-2 mt-0.5 flex-shrink-0" />
                            <p className="text-xs sm:text-sm text-yellow-800">
                              You haven't uploaded a resume to your profile. Consider uploading one or attach a custom resume below.
                            </p>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
                      ...
                      <div className="mb-6 sm:mb-8">
                        <div className="flex items-center justify-between mb-3 sm:mb-4">
                          <h4 className="font-medium text-gray-700 text-sm sm:text-base">Required Skills</h4>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                          {jobSkills.map((skill, index) => (
                            <div key={index} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-gray-50 rounded-lg gap-2">
                              <span className="font-medium text-gray-900 text-sm sm:text-base">{skill}</span>
                              <div className="flex items-center justify-end sm:justify-start">
                                <div className="flex items-center flex-wrap">
                                  {renderStars(skill, formData.skill_ratings[skill] || 0)}
                                </div>
                                <span className="ml-2 text-xs sm:text-sm text-gray-600 min-w-[45px] text-right">
                      ...
                      <div className="mb-6">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2">
                          <h4 className="font-medium text-gray-700 text-sm sm:text-base">Additional Skills</h4>
                          <button
                            type="button"
                            onClick={handleAddCustomSkill}
                            className="flex items-center text-sm text-primary-600 hover:text-primary-700 self-start sm:self-auto"
                          >
                            ...
                            {formData.custom_skills.map((skill, index) => (
                              <div key={index} className="flex items-center justify-between p-3 bg-green-50 border border-green-100 rounded-lg">
                                <div className="flex-1 min-w-0">
                                  <span className="font-medium text-gray-900 text-sm sm:text-base truncate">{skill.name}</span>
                                  {skill.is_new && (
                                    <span className="ml-2 text-xs px-2 py-1 bg-green-100 text-green-800 rounded-full whitespace-nowrap">
                                      New
                                    </span>
                                  )}
                                </div>
                      ...
                      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
                        <h3 className="text-base sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6">Additional Information</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                          {/* Driving License */}
                          <div className="flex items-start space-x-3 p-3 sm:p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            ...
                            {formData.driving_license ? (
                              ...
                              <XCircle className="h-5 w-5 sm:h-6 sm:w-6 text-gray-300 flex-shrink-0" />
                            )}
                          </div>
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              className="w-full p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition"
                            />
                    ...
                    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
                      ...
                      <textarea
                        ...
                        rows={8}
                        className="w-full p-3 sm:p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition resize-none text-sm"
                    ...
                    {/* Resume Selection */}
                    <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3 sm:mb-4">Resume</h3>

                      {/* Option 1: Use profile resume */}
                      {profile?.resume && (
                        <div className="mb-3 sm:mb-4">
                          <label className="flex items-start p-3 sm:p-4 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
                            <input
                              type="radio"
                            ...
                            <div className="ml-3 flex-1">
                              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                <div>
                                  <span className="font-medium text-gray-900 text-sm sm:text-base">Use my profile resume</span>
                                ...
                                <a
                                  ...
                                  className="text-xs sm:text-sm text-primary-600 hover:text-primary-700 whitespace-nowrap"
                      ...
                      <div>
                        <label className="flex items-start p-3 sm:p-4 border border-gray-300 rounded-lg hover:bg-gray-50 cursor-pointer">
                          ...
                          />
                          <div className="ml-3 flex-1">
                            <span className="font-medium text-gray-900 text-sm sm:text-base">Upload a different resume</span>
                                  ...
                                  <div className="text-center">
                                    ...
                                    <div className="flex flex-col items-center space-y-2 sm:space-y-3">
                                      ...
                                      <label
                                        ...
                                        className="px-4 sm:px-6 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors cursor-pointer font-medium text-sm"
                                      >
                                        Choose File
                                      </label>
              ...
              <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div className="order-2 sm:order-1">
                    <div className="text-xs sm:text-sm text-gray-600">Application Score</div>
                    <div className="flex items-center space-x-2">
                      <div className="text-xl sm:text-2xl font-bold text-blue-700">{matchScore}%</div>
                  ...
                  <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-4 order-1 sm:order-2">
                    <button
                      ...
                      className="px-4 sm:px-6 py-2.5 sm:py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium text-sm"
                    >
                      Cancel
                    </button>
                    <button
                      ...
                      className="px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium flex items-center justify-center shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-sm"
                    >
                      {isSubmitting ? (
          ...
          <div className="fixed inset-0 z-[60] overflow-y-auto">
            <div className="flex min-h-screen items-center justify-center p-4">
              ...
              <motion.div
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-5 sm:p-6 mx-4"
              >
                ...
                <div className="mb-5 sm:mb-6">
                  ...
                  <ul className="space-y-2">
                    {unratedSkills.map((skill, index) => (
                      <li key={index} className="flex items-center text-sm sm:text-base text-gray-800">
                        <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-gray-400 rounded-full mr-2 sm:mr-3"></span>
                        {skill}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
                  <button
                    type="button"
                    onClick={() => setShowUnratedSkillsModal(false)}
                    className="px-4 sm:px-5 py-2 sm:py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium text-sm"
                  >
                    Go Back & Rate Skills
                  </button>
                  <button
                    ...
                    disabled={isSubmitting}
                    className="px-4 sm:px-5 py-2 sm:py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium flex items-center justify-center text-sm"
                  >
                    {isSubmitting ? (
                      <span className="flex items-center">
                        <Loader2 className="animate-spin h-4 w-4 mr-2" />
